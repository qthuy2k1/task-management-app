...
func TestGetAllTaskCategories(t *testing.T) {
	...
	taskCategoryServiceMock.On("GetAllTaskCategories", mock.Anything, mock.Anything).Return(taskCategoryList, nil)
	...
	r.Get("/taskCategories", func(w http.ResponseWriter, r *http.Request) {
		...
		taskCategoryList, err := taskCategoryServiceMock.GetAllTaskCategories(r, tokenAuth)
		if err != nil {
		...
		render.JSON(w, r, taskCategoryList.TaskCategories)
	})
	...
	expectedBody, _ := json.Marshal(taskCategoryList.TaskCategories)
	...
	taskCategoryServiceMock.AssertCalled(t, "GetAllTaskCategories", mock.Anything, mock.Anything)
}
func TestGetTaskCategoryByID(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			taskCategoryServiceMock.On("GetTaskCategoryByID", tt.taskCategoryID, mock.AnythingOfType("*http.Request"), mock.AnythingOfType("*jwtauth.JWTAuth")).Return(tt.mockTaskCat, tt.mockErr)
			...
			router.Get("/task-categories/{taskCategoryID}", func(w http.ResponseWriter, r *http.Request) {
				...
				taskCat, err := taskCategoryServiceMock.GetTaskCategoryByID(id, r, tokenAuth)
				if err != nil {
				...
				if err := render.Render(w, r, &taskCat); err != nil {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			router.Post("/task_categories", func(w http.ResponseWriter, r *http.Request) {
				...
				err = taskCategoryServiceMock.AddTaskCategory(&taskCategoryData, r, tokenAuth, token)
				...
				if err := render.Render(w, r, &taskCategoryData); err != nil {
					render.Render(w, r, ServerErrorRenderer(err))
					return
				}
			})
			...
			taskCategoryServiceMock.On("AddTaskCategory", tc.taskCategory).Return(nil)
...
func TestDeleteTaskCategory(t *testing.T) {
	...
	router.Delete("/task-categories/{taskCategoryId}", func(w http.ResponseWriter, r *http.Request) {
		...
		err = taskCategoryServiceMock.DeleteTaskCategory(taskCategoryId, r, tokenAuth, token)
		if err != nil {
			if err == db.ErrNoMatch {
				render.Render(w, r, ErrNotFound)
			} else {
	...
	taskCategoryServiceMock.On("DeleteTaskCategory", mock.AnythingOfType("int")).Return(nil)
	...
	taskCategoryServiceMock.AssertCalled(t, "DeleteTaskCategory", 1)
}
...
func TestUpdateTaskCategoryHandler(t *testing.T) {
	...
	r.Put("/task-categories/{taskCategoryID}", func(w http.ResponseWriter, r *http.Request) {
		...
		taskCategory, err := taskCategoryServiceMock.UpdateTaskCategory(taskCategoryID, taskCategoryData, r, tokenAuth, token)
		if err != nil {
			if err == db.ErrNoMatch {
				render.Render(w, r, ErrNotFound)
			} else {
	...
	taskCategoryServiceMock.On("UpdateTaskCategory", 1, taskCategoryData).Return(expectedTaskCategory, nil)
